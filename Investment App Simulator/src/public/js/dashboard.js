...
//                 throw new Error(`Failed to show news: ${errorData.error}`);
//             }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchNews(category = "general") {
        try {
            const response = await fetch(`/dashboard/news?category=${category}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                }
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(`Failed to fetch news: ${errorData.error}`);
            }
    ...
    // Render news items dynamically
    async function renderNews(filterTags = []) {
        const newsItems = await fetchNews(); // Fetch news from backend
        ...
        filteredNews.forEach((news) => {
            const { news_id, image_url, title, caption, tags, category } = news;
            const taskListId = categoryIds[category] || 'important-list'; // Default to "important" if unknown category
    ...
    // Update tag counts based on fetched news
    function updateTagCounts(newsItems) {
        const tagCounts = {}; // Map to store tag counts
        ...
        // Update counts in the filter area
        document.querySelectorAll('.tag-count').forEach(tagElement => {
            const tag = tagElement.dataset.countTag;
            tagElement.textContent = `(${tagCounts[tag] || 0})`;
        });
    }
    ...
    function initializeDragAndDrop() {
        ...
        async function drop() {
            ...
            const newsId = task.dataset.id;
            if (!newsId) {
                console.error('news_id is missing.');
                return;
            }
